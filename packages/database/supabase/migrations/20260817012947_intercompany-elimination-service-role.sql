-- Fix-forward on 20260816162947: generateEliminationEntries writes to the
-- ELIMINATION ENTITY's company (accountingPeriod, journal, journalLine), a
-- synthetic consolidation company the triggering user is not an
-- employee-with-permission of. As SECURITY INVOKER called with the user's
-- RLS-scoped client, the accountingPeriod INSERT policy
--   (companyId = ANY get_companies_with_employee_permission('accounting_create'))
-- rejected the get-or-create period insert with
--   "new row violates row-level security policy for table \"accountingPeriod\"".
--
-- The function stays SECURITY INVOKER ON PURPOSE: a normal user who calls
-- client.rpc('generateEliminationEntries', ...) directly then runs under their
-- own RLS and simply cannot write to the elimination entity, so RLS itself is
-- the backstop against generating eliminations for a group they lack access to.
-- Making it SECURITY DEFINER would run privileged for ANY caller and rest the
-- whole boundary on the in-function membership check — a broader surface we
-- don't want on a PostgREST-exposed RPC.
--
-- Instead the ERP route (intercompany.eliminate.tsx) verifies access with
-- requirePermissions({ create: "accounting", bypassRls: true }) and passes the
-- service-role client, which bypasses RLS at the role level. The only change the
-- function needs is the membership gate: it read auth.uid(), which is NULL under
-- the service role (no user sub in the service-role JWT) and would raise
-- "Insufficient permissions". It now checks p_user_id — the authenticated user
-- the route already passes — so the group-membership defense-in-depth check holds
-- for the service-role path.
--
-- The body is otherwise byte-for-byte identical to 20260816162947; only the
-- membership check's identity source changes (auth.uid()::text -> p_user_id).
...
BEGIN
  -- Check that the authenticated user (passed by the route, verified there via
  -- requirePermissions) belongs to at least one company in this group. Uses
  -- p_user_id, not auth.uid(): the route invokes this with the service-role
  -- client, whose JWT has no user sub, so auth.uid() would be NULL here.
  IF NOT EXISTS (
    SELECT 1
