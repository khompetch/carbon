-- get_purchasing_planning's demand_data union read only demandActual + demandForecast,
-- so a purchased item whose only demand is a planner-entered demandProjection
-- (the "Demand Forecast" drawer writes the demandProjection table) never appeared
-- in purchasing planning and never drove Qty to Order. The identical fix was
-- applied to get_production_planning in 20260715195226 but not to its purchasing
-- twin. Fork the current definition (20260618171234) verbatim and add the third
-- demandProjection arm to demand_data. Read-only function change; no schema/table
-- change. MRP recalc is NOT required for a forecast to appear once this lands.
