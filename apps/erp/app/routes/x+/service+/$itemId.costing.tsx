...
export async function loader({ request, params }: LoaderFunctionArgs) {
  ...
  if (itemCost.error) {
    throw redirect(
      ...
      await flash(
        request,
        error(itemCost.error, "Failed to load service costing")
      )
    );
  }
...
export async function action({ request, params }: ActionFunctionArgs) {
  ...
  if (updateItemCost.error) {
    throw redirect(
      ...
      await flash(
        request,
        error(updateItemCost.error, "Failed to update service costing")
      )
    );
  }

  throw redirect(
    path.to.serviceCosting(itemId),
    await flash(request, success("Updated service costing"))
  );
}

export default function ServiceCostingRoute() {
