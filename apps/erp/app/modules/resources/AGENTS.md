...
- `getAbilities` / `getAbility` / `getEmployeeAbilities` / `getEmployeeAbility` / `insertAbility` / `updateAbility` / `upsertEmployeeAbility` / `insertEmployeeAbilities` — skill tracking (`makeAbilityCurve` in `resources.models.ts` builds the curve JSON from startingPoint/weeks)
