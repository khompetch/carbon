...
- **OEE Dashboard** — `/x/production/oee` (route `x+/production+/oee.tsx` + resource route `api+/production.oee.ts`, components in `ui/Oee/`). Availability from productionEvent runtime (minus ALL recorded `workCenterDowntime` + maintenance intervals — downtime wins over a still-running event) vs location-shift planned time minus PLANNED downtime (maintenance `oeeImpact` Down/Planned + Planned downtime records); Performance from `makeDurations` per-piece standards vs runtime (no setup credit); Quality from productionQuantity. Semantics match the hourly board. Spec: `.ai/specs/2026-07-09-oee-dashboard.md`.
...
- **Downtime recording** — `downtimeReason` (name + `downtimeType` Planned/Unplanned) configured at `/x/production/downtime-reasons` (`ui/DowntimeReasons/`); `workCenterDowntime` intervals recorded from the MES operation screen (open interval = `endTime` NULL; auto-closed when a production event starts). Recorded downtime of any kind subtracts from OEE runtime (downtime wins over events); residual in-shift idle counts as unplanned.
- **Auto no-output downtime** — when a work center has an open production event but no output within `multiplier × cycle time`, the Inngest cron `detect-no-output-downtime` (every minute) inserts an Unplanned `workCenterDowntime` row (`isAuto: true`) with the company's default reason, and the OEE boards also detect it live (`detectNoOutput` in `@carbon/utils`). Config: `companySettings.autoDowntimeMultiplier` + `autoDowntimeReasonId` (Settings → Production; NULL = off) with per-WC override `workCenter.autoDowntimeMultiplier` (WorkCenterForm; 0 = disabled). Logging output in MES closes ALL open downtime on the work center immediately — auto and manual alike, same as starting a production event (`endOpenDowntimeForOperation` from the three insert-quantity fns). Spec: `.ai/specs/2026-07-16-oee-work-center-hourly.md`.
